...
def gf(m, poly_str):
    ...
    See the implementation of gf_packed() for a more compact implementation
...
def gf_packed(m, poly_str, dtype=np.uint32):
    """Generate the GF(2^m) extension field elements as polynomials over GF(2)

    Same as the gf() function but representing each GF(2^m) element as an int
    with packed bits.
...
def elem_to_index_lut(gf_table):
    ...
    "alpha^i" of the primitive element alpha. Multiplication of two elements
    "alpha^i1" and "alpha^i2" is greatly simplified by computing "alpha^i1 *
    alpha^i2 = alpha^(i1 + i2)". For that, it is useful to generate a look-up
    table (LUT) mapping each GF(2^m) element to its exponent i.

    This function returns a LUT to facilitate this mapping. For each element
    alpha^i, instead of mapping to the exponent "i", the generated LUT maps to
    the element's index on the original GF table. Since the GF table produced
    by `gf_packed()` has element alpha^0 at index=1, alpha^1 at index=2, and so
    on, this function generates an array whose value at position "alpha^i" is
    the index "i+1", i.e., "x[alpha^i] = i + 1".
    ...
    ----------
    gf_table : np.array
        A (2^m by 1) array with the GF(2^m) elements [0, alpha^0, alpha^1, ...,
        alpha^(2^m -2)] in this order as produced by the gf_packed() function.
...
def multiply(gf_table, inv_gf_table, a, b):
    ...
    gf_table : np.array
        A (2^m by 1) array with the GF(2^m) elements [0, alpha^0, alpha^1, ...,
        alpha^(2^m -2)] in this order as produced by the gf_packed() function.
    inv_gf_table : np.array
        A (2^m by 1) array produced by function `elem_to_index_lut()`, which
        maps each GF(2^m) element alpha^i to its index "i + 1".
    a : gf_table.dtype
        Multiplicand.
    b : gf_table.dtype
        Multiplier.
    ...
    if a == 0 or b == 0:
        return 0
    two_to_m_minus_one = len(gf_table) - 1  # same as 2^m - 1
    exp_a = inv_gf_table[a] - 1  # exponent ia of a represented as alpha^ia
    exp_b = inv_gf_table[b] - 1  # exponent ib of b represented as alpha^ib
    return gf_table[((exp_a + exp_b) % two_to_m_minus_one) + 1]


def inverse(gf_table, inv_gf_table, a):
    ...
    """
    two_to_m_minus_one = len(gf_table) - 1  # same as 2^m - 1
    exp_a = inv_gf_table[a] - 1
    return gf_table[((two_to_m_minus_one - exp_a) % two_to_m_minus_one) + 1]


def divide(gf_table, inv_gf_table, a, b):
    ...
    """
    return multiply(gf_table, inv_gf_table, a,
                    inverse(gf_table, inv_gf_table, b))


def multiply_gf_poly(gf_table, inv_gf_table, a, b):
    ...
    for i, a_i in enumerate(a):
        for j, b_j in enumerate(b):
            ...
            prod[i + j] ^= multiply(gf_table, inv_gf_table, a_i, b_j)
    return prod
...
def min_polynomial(gf_table, inv_gf_table, beta):
    ...
    m = int(np.log2(len(gf_table)))  # dimension of the field.
    exp_beta = inv_gf_table[beta] - 1  # exponent i of alpha^i = beta
    conjugate_list = conjugates(m, exp_beta)  # exponents i^(2^l)

    # Multiply the terms "(x + beta^(2^l))" iteratively
    prod = [gf_table[1]]  # start with the unitary element
    for exp in conjugate_list:
        a = [gf_table[1], gf_table[exp + 1]]  # x + beta^(2^l)
        prod = multiply_gf_poly(gf_table, inv_gf_table, a, prod)
    ...
    return [inv_gf_table[x] for x in prod]
