...
enum class bch_variant : uint8_t {
    standard,    // standard BCH
    even_weight, // force even parity: g(x) <- g(x) * (x + 1)
};
...
public:
     ...
     */
    bch_codec(const galois_field<T>* const gf,
              uint8_t t,
              uint32_t n = 0,
              bch_variant variant = bch_variant::standard);
