 ...
 * @param variant BCH code variant.
 * @return gf2_poly<P> Generator polynomial.
 */
template <typename T, typename P>
gf2_poly<P>
compute_gen_poly(const galois_field<T>* const gf, uint8_t t, bch_variant variant)
{
    ...
    if (variant == bch_variant::even_weight) {
        gf2_poly<P> even_weight_term(0b11); // x + 1
        g = g * even_weight_term;
    }
...
template <typename T, typename P>
bch_codec<T, P>::bch_codec(const galois_field<T>* const gf,
                           uint8_t t,
                           uint32_t n,
                           bch_variant variant)
    : m_gf(gf),
      m_t(t),
      m_g(std::move(compute_gen_poly<T, P>(gf, t, variant))),
...
// Encode into type T
template <typename T, typename P>
T _encode(const T& msg, const gf2_poly<P>& g, T msg_mask, uint32_t n_parity_bits)
{
