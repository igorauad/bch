 ...
 *
 * Copyright (c) 2022-2026 Igor Freire
 *
 * This file was originally published under the GNU General Public License v3
 * (GPLv3) as part of project gr-dvbs2rx. This copy is provided under the MIT
 * License.
 */
#include <bch/bch.hpp>
...
void check_err_free_syndrome(const bch_codec<T, P>& codec)
{
...
template <typename T, typename P>
void check_decode(const bch_codec<T, P>& codec, uint8_t num_errors = 0)
{
    T max_msg = (1 << codec.get_k()) - 1;
    for (T msg = 0; msg <= max_msg; msg++) {
...
BOOST_AUTO_TEST_CASE(test_bch_ccsds_231_0_b3)
{
    // BCH code from CCSDS 231.0-B-3: modified even-weight BCH code
    gf2_poly_u64 prim_poly(0b1000011); // x^6 + x + 1
    galois_field gf(prim_poly);
    bch_codec<uint64_t, uint64_t> codec(&gf, /*t=*/1, /*n=*/0, bch_variant::even_weight);

    // Check the code configuration given on Section 3.3.2 of the spec
    gf2_poly_u64 expected_gen_poly =
        gf2_poly<uint64_t>(0b11000101); // x^7 + x^6 + x^2 + 1
    BOOST_CHECK(codec.get_gen_poly() == expected_gen_poly);
    BOOST_CHECK_EQUAL(codec.get_n(), 63);
    BOOST_CHECK_EQUAL(codec.get_k(), 56);

    // Confirm the error correction capability
    ...
    // Confirm uncorrectable status with bit errors exceeding the error
    // correction capability
    const uint64_t msg_mask = (1ULL << codec.get_k()) - 1;
    const uint64_t msg = random_u64() & msg_mask;
    const uint64_t tx_codeword = codec.encode(msg);
    constexpr uint32_t num_bit_errors = 4;
    const uint64_t rx_codeword =
        flip_random_bits(tx_codeword, codec.get_n(), num_bit_errors);
    int corrected_bits;
    const uint64_t decoded_msg =
        codec.decode(rx_codeword, corrected_bits, /*verify=*/true);
    BOOST_CHECK_NE(tx_codeword, rx_codeword); // error inserted
    BOOST_CHECK_NE(decoded_msg, msg);         // residual errors
    BOOST_CHECK_EQUAL(corrected_bits, -1);    // uncorrectable errors
}
